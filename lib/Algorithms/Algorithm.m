...
nx = length(x);
nq = number of configuration (velocity) variables;
q = ???;
qd = ???;
%   * the state x = (q, qd)
if nargin < 4 || isempty(taufun)
    tau = a zero vector of appropriate size;
else
    tau = the result of calling taufun
%       * make sure to get the calling convention right (see function's
%         header comments for details)
end
xdot = ???
%   * the derivative of the state xdot is xdot = (qd, qdd)
%   * given obj, how can we compute qdd?
end